...
public class HttpUtil {
            ...
            try {
                if (conn.getResponseCode() == status) {
                    ...
                    while ((lines = reader.readLine()) != null) {
                        ...
                        FileUtil.appendFile("E:\\html_fetch\\echuzhou",lines);
                    }
                    reader.close();
    ...
    public static String getSpecialCookie(String method, String url, Map<String, ?> paramMap, Map<String, String> headMap, int status, String encode,String cookieStr) throws HttpException {
        HttpURLConnection conn = null;
        try {
            String params = getParams(paramMap, encode);;
            if (GET_METHOD.equals(method)) {
                String split = "";
                if (StringUtils.isNotBlank(params)){
                    split = url.contains("?") ? "&" : "?";
    ...
    public static Map<String,String> getParamsMap(String reqUrl, String encode)
        ...
        String paramsStr = reqUrl;
        if(paramsStr.contains("?")){
        	paramsStr = reqUrl.substring(reqUrl.indexOf("?")+1);
        }
    ...
    public static String doProxyHttp(String method, String url, Map<String, ?> paramMap, Map<String, String> headMap, int status, String encode,String ip,Integer port) throws HttpException {
        ...
        try {
            ...
            if (GET_METHOD.equals(method)) {
                ...
                conn = (HttpURLConnection) new URL(url + split + params).openConnection(proxy);
//                conn.setRequestProperty("Content-type", "text/html;charset=" + encode);
            } else {
                conn = (HttpURLConnection) new URL(url).openConnection();
//                conn.setRequestProperty("Content-type", "application/x-www-form-urlencoded");
